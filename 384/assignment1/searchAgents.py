...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        "the state representation includes a position and a list of which corners have been visited, in order"
        startState = []
        cornersVisited = (False, False, False, False)
        visitedList = list(cornersVisited)
        startState.append(self.startingPosition)
        startState.append(visitedList)
        return startState
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        visited = state[1]
        "check list: if any corners have not been visited, return false. otherwise return true."
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                "for each successor, create state including position and list of visited corners"
                successorState = []
                successorPosition = (nextx, nexty)
                successorAction = action
                successorCost = 1
                "copy the list of corners visited, so that we won't change the original"
                visited = list(state[1])
                for corner in self.corners:
                    if corner == successorPosition:
                        index = self.corners.index(corner)
                        visited[index] = True
                successorState.append((successorPosition, visited))
                "also includes the action and respective cost to the successor in the return value"
                successorState.append(successorAction)
                successorState.append(successorCost)
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    "list to store the heuristic value for each corner"
    cornersHeuristics = []
    "store the number of corners visited in this state for use with heuristic calculation"
    visitedCount = 0
    for corner in corners:
        index = corners.index(corner)
        "create heuristic for each corner based on if its been visited or not"
        if state[1][index] == False:
            "use distance to non-visited corners as heuristic value"
            xy1 = state[0]
            xy2 = corner
            distance = abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
            cornersHeuristics.append(distance)
        else:
            "corner already visited. should have a high heuristic value so that it isn't considered again"
            visitedCount += 1
            cornersHeuristics.append(999)
        
    "return smallest corner heuristic as the heuristic for this position"
    smallest = 999
    for value in cornersHeuristics:
        if value < smallest:
            smallest = value
    if visitedCount == 4:
        "all corners have been visited. this is a goal state. heuristic value should be 0"
        smallest = 0
...
def foodHeuristic(state, problem):
    ...
    "the lesser the amount of food left to be eaten, the better the heuristic"
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        """
        here I just used my same code for breadthFirstSearch. 
        combined with the isGoalProblem of AnyFoodSearchProblem, it will always find the closest food.
        """
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        "compare state to food list. if it is food, it is a goal state."
